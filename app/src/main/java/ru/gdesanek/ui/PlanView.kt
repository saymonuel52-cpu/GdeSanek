                ...
                if (event.pointerCount == 1) {
                    when (currentTool) {
                        Tool.EDIT -> {
                            ...
                            dragObject?.let { dragObj -> for (t in tracks) { if (t.points.isNotEmpty()) { val fp = t.points.first(); if (sqrt((fp.x - dragObj.x) * (fp.x - dragObj.x) + (fp.y - dragObj.y) * (fp.y - dragObj.y)) < 80f) dragLinkedTracks.add(t.id) } } }
                            dragWall = hitW
