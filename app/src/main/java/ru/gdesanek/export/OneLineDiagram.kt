...
object OneLineDiagram {
    ...
    fun buildGroups(
        ...
        tracks.forEachIndexed { idx, tr ->
            ...
            var powerW = 0f
            var hasSocket = false; var hasLight = false; var isWet = false
            for (o in mine) {
                powerW += (ru.gdesanek.theme.SymbolPalette.power(o.type)?.toFloat() ?: 0f)
                val t = o.type
                if (t.contains("socket")) hasSocket = true
                if (t.contains("lamp")) hasLight = true
                val nm = o.name.lowercase()
                if (nm.contains("стир") || nm.contains("сан") || nm.contains("ван") || nm.contains("душ")) isWet = true
            }
            val top = mine.maxByOrNull { (ru.gdesanek.theme.SymbolPalette.power(it.type)?.toFloat() ?: 0f) }
            ...
            val suffix = when {
                hasSocket && !hasLight -> "с"
                hasLight && !hasSocket -> "о"
                else -> ""
            }
            var len = 0f
    ...
    fun render(
        ...
        canvas.drawLine(boxX + 30f, busY + 40f, boxX + 30f + busLen, busY + 40f, busPaint)
