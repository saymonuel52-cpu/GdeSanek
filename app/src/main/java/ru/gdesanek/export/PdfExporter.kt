...
object PdfExporter {
    ...
    fun export(
        ...
        val page2 = document.startPage(PdfDocument.PageInfo.Builder(pw, ph, 1).create())
            ...
            canvas.drawText("ОБЩИЕ ДАННЫЕ", M, M + 8f, titlePaint)
            
            val textPaint = Paint().apply { color = Color.BLACK; textSize = 14f }
            var y = M + 40f
            
            canvas.drawText("ВЕДОМОСТЬ РАБОЧИХ ЧЕРТЕЖЕЙ:", M, y, textPaint); y += 24f
            canvas.drawText("1.1  Общие данные", M, y, textPaint); y += 24f
            canvas.drawText("2.1  Однолинейная схема щита ЩР-1", M, y, textPaint); y += 24f
            canvas.drawText("3.1  План расположения ЭО и освещения", M, y, textPaint); y += 48f
            
            canvas.drawText("ПОЯСНИТЕЛЬНАЯ ЗАПИСКА:", M, y, textPaint); y += 24f
            val notes = listOf(
            ...
            for (note in notes) { canvas.drawText(note, M, y, textPaint); y += 24f }
            y += 24f
            
            canvas.drawText("ВЕДОМОСТЬ ССЫЛОЧНЫХ ДОКУМЕНТОВ:", M, y, textPaint); y += 24f
            val docs = listOf(
            ...
            for (doc in docs) { canvas.drawText(doc, M, y, textPaint); y += 24f }
            ...
            val doc2 = passport.getOrNull(0)?.takeIf { it.isNotBlank() } ?: "ЭОМ"
            canvas.drawText(org, sx + 6f, sy + 18f, st)
            canvas.drawText("$doc2   Лист 1.1   Общие данные", sx + 6f, sy + 42f, st)
